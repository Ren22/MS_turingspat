from sympy import *
import sympy
import numpy as np

a, b, c, d, au, u0, v0, n, M, u, v, m = symbols('a, b, c, d, au, u0, v0, n, M, u, v, m ')

f = a * (u - u0) * M ** n + b * (v - v0) * M ** n - (au * (u - u0) ** 3)
g = c * (u - u0) * M ** n + d * (v - v0) * M ** n

fu = diff(f, u)
fv = diff(f, v)
gu = diff(g, u)
gv = diff(g, v)

# print('fu = {0}, fv = {1}, gu ={2}, gv = {3}'.format(fu, fv, gu, gv))

numerator = (sympy.sqrt(fu * gv - fv * gu) + sympy.sqrt(-fv * gu)) ** 2
denominator = (fu ** 2)

print ('numerator/denominator = {0}'.format(numerator / denominator))
M = 100. * (1. / 10)
result = numerator / denominator
result_subs = result.subs([(a, 0.2), (b, -0.4), (c, 0.6), (d, -0.8), (au, 5.), (u0, 0.5), (v0, 0.5), (n, -2.)])
print(result_subs)
#
# for size in np.arange(10., 120., 1.):
#     discretizing_factor = 6.
#     size_segments = discretizing_factor * size
#     x = np.linspace(0., size, size_segments)
#     result_subs = result.subs([(a, 0.2), (b, -0.4), (c, 0.6), (d, -0.8),
#                                (au, 5.), (u0, 0.5), (v0, 0.5), (n, -2.), (m, 100. * (1. / x[-1]))])
#     print(result_subs)
