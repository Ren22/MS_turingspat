...
def wave_search(size, size_segments, time_period, times, au, Du):
    ...
    u = rxd.Species(where, d=1, initial=0)  # activator
    z = rxd.Species(where, d=20, initial=0)  # inhibitor
    ...
    bistable_reaction1 = rxd.Rate(u, (a * u + b * z))
    bistable_reaction2 = rxd.Rate(z, (c * u + d * z))

    # initial conditions
    h.finitialize()
    for node in u.nodes:
        if node.x < .2: node.concentration = 1
        if node.x > .8: node.concentration = 1
    for node in z.nodes:
        if node.x < .2: node.concentration = 1
        if node.x > .8: node.concentration = 1
